...
tests = return
  ...
  , test 0.5 "swap" (swap :: (Int,Bool) -> (Bool,Int))
  ...
  , test 0.5 "transpose-p-p" (transpose :: Unop (Pair (Pair Bool)))
  , test 2.0 "transpose-p-rt3" (transpose :: Pair (RTree N3 Bool) -> RTree N3 (Pair Bool))
  , test 15  "transpose-rt3-rt4" (transpose :: RTree N3 (RTree N4 Bool) -> RTree N4 (RTree N3 Bool))
  , test 0.5 "dot" ((<.>) :: Pair Int -> Pair Int -> Int)
  , test 1.0 "dotG-p-rt3" (dotG :: Pair (RTree N3 Int) -> Int)
  , test 1.0 "dot-rt3" ((<.>) :: RTree N3 Int -> RTree N3 Int -> Int)
  ...
  , test 0.5 "lsums-p" (lsums :: Pair Int -> (Pair Int, Int))
  , test 1.5 "lsums-rt4" (lsums :: RTree N4 Int -> (RTree N4 Int, Int))
  , test 3.0 "lsums-lt6" (lsums :: LTree N6 Int -> (LTree N6 Int, Int))
  , test 0.5 "lsums-v6" (lsums :: Vec N6 Int -> (Vec N6 Int, Int))
  , test 1.5 "powers-rt4" (powers :: Int -> RTree N4 Int)
  , test 1.5 "evalPoly-rt4" (evalPoly :: RTree N4 Int -> Int -> Int)
  , test 1.0 "fft-p" (fft :: Unop (Pair C))
  , test 2.5 "fft-lt3" (fft :: LTree N3 C -> RTree N3 C)
  , test 2.0 "fft-rt2" (fft :: RTree N2 C -> LTree N2 C)
  ]
